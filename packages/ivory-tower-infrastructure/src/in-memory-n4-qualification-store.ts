...
export class InMemoryN4QualificationStore implements N4QualificationStore {
    ...
    async addSourceToProject(projectId: string, contentHash: string): Promise<void> {
        if (!this.projects.has(projectId)) {
            throw new Error(`Unknown N4 project: ${projectId}`);
        }
    ...
    async saveAnchor(anchor: N4AnchorRecord): Promise<N4AnchorRecord> {
        if (!this.projects.has(anchor.projectId)) {
            throw new Error(`Unknown N4 project: ${anchor.projectId}`);
        }
        if (!this.representations.has(anchor.representationId)) {
            throw new Error(`Unknown N4 representation: ${anchor.representationId}`);
        }
        const existing = this.anchors.get(anchor.id);
        if (existing !== undefined) {
            if (JSON.stringify(existing) !== JSON.stringify(anchor)) {
                throw new Error(`N4 anchor is immutable: ${anchor.id}`);
            }
            return copy(existing);
        }
