...
interface AnchorRow {
    ...
    source_version_id: string;
    artifact_id: string;
    spans: N4AnchorRecord['spans'];
    quote: N4AnchorRecord['quote'];
    coordinates: N4AnchorRecord['coordinates'];
    confidence: N4AnchorRecord['confidence'];
    created_at: Date;
}
const representation = (row: RepresentationRow): N4RepresentationRecord => ({
    ...
    converterRef: row.converter_ref,
    text: row.text,
...
export class PostgresN4QualificationStore implements N4QualificationStore {
    ...
    async ensureProject(project: N4ProjectRecord): Promise<N4ProjectRecord> {
        const result = await this.pool.query<{ id: string; name: string; created_at: Date }>(
            `INSERT INTO ivory_n4_projects (id, name, created_at) VALUES ($1, $2, $3)
             ON CONFLICT (id) DO UPDATE SET id = ivory_n4_projects.id RETURNING *`,
            [project.id, project.name, project.createdAt],
        );
        const row = result.rows[0];
        if (!row) {
            throw new Error(`N4 project was not persisted: ${project.id}`);
        }
    ...
    async listProjectSourceHashes(projectId: string): Promise<readonly string[]> {
        const result = await this.pool.query<{ content_hash: string }>(
            'SELECT content_hash FROM ivory_n4_project_sources WHERE project_id = $1 ORDER BY content_hash',
            [projectId],
        );
    ...
    async persistRepresentation(value: N4RepresentationRecord): Promise<N4RepresentationRecord> {
        ...
        const row = result.rows[0];
        if (!row) {
            throw new Error(`N4 representation was not persisted: ${value.id}`);
        }
        return representation(row);
    }
    async getRepresentation(id: string): Promise<N4RepresentationRecord | undefined> {
        const result = await this.pool.query<RepresentationRow>('SELECT * FROM ivory_n4_representations WHERE id = $1', [id]);
        return result.rows[0] ? representation(result.rows[0]) : undefined;
    }
    async saveAnchor(value: N4AnchorRecord): Promise<N4AnchorRecord> {
        ...
        const row = result.rows[0];
        if (!row) {
            throw new Error(`N4 anchor was not persisted: ${value.id}`);
        }
        return anchor(row);
    }
