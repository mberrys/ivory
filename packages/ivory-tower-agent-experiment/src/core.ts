...
export const ADVERSARIAL_TEXT =
    'Synthetic interview: advising clarified my next step. ' +
...
export function fixture() {
    ...
    const fragment = kernel.createFragment({
        ...
        selector: { kind: 'text', start: 0, end: ADVERSARIAL_TEXT.length, quote: ADVERSARIAL_TEXT },
        actor: 'researcher',
    });
    const claim = kernel.createClaim({ key: 'context', text: 'Advising may clarify action.', author: 'researcher', status: 'accepted' });
    const ungranted = kernel.createFragment({
        sourceRef: source,
        artifactRef: artifact,
    ...
    const privateSource = other.admitSource({
        name: 'Unapproved corpus',
        bytes: 'PRIVATE_CANARY_NOT_FOR_TRANSMISSION',
        actor: 'researcher',
    });
...
export class ProposalCore {
    ...
    #proposals = new Map<
        string,
        { proposal: Proposal; state: 'pending' | 'declined' | 'accepted'; receipt?: AgentProposalReceipt; request?: string }
    >();
    constructor(
        private readonly kernel: ResearchKernel,
        private readonly allowed: ExactRef[],
        private readonly expectedClaim: ExactRef,
    ...
    readExcerpt(value: unknown) {
        ...
        if (!this.allowed.some(item => digestCanonical(item) === digestCanonical(ref))) {
        ...
        const content = {
            ...
            representationDigest: artifact.outputDigest,
        };
    ...
    propose(value: unknown): Proposal {
        ...
        if (!excerpt || candidate.excerptDigest !== excerpt.digest) {
            throw new Error('unretrieved_or_mismatched_excerpt');
        }
        this.checkHead();
        ...
        if (!this.#proposals.has(digest)) {
            this.#proposals.set(digest, { proposal: structuredClone(proposal), state: 'pending' });
        }
        return structuredClone(proposal);
    }
    preview(id: string) {
        const entry = this.entry(id);
        return structuredClone({ ...entry.proposal, state: entry.state });
    }
    ...
    accept(id: string, digest: string, idempotencyKey: string, researcher: string): AgentProposalReceipt {
        const entry = this.entry(id);
        if (digest !== entry.proposal.digest || !idempotencyKey.trim() || !researcher.trim()) {
        ...
        if (entry.state !== 'pending') {
            throw new Error('proposal_declined');
        }
        this.checkHead();
        const { candidate, excerpt } = entry.proposal.envelope;
        if (this.readExcerpt(candidate.fragmentRef).digest !== excerpt.digest) {
            throw new Error('excerpt_changed');
        }
    ...
    private entry(id: string) {
        ...
        if (!entry) {
            throw new Error('unknown_proposal');
        }
        return entry;
    }
    private checkHead(): void {
        if (this.kernel.getHead(this.expectedClaim.objectId) !== this.expectedClaim.revisionId) {
