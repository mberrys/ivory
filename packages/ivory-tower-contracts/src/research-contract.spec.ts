...
import { citationResolutionResponseSchema, fixtureResetRequestSchema, projectEditConflictSchema,
    projectEditRequestSchema, projectOpenResponseSchema, resolvedRunSpecSchema,
    runSpecResolutionResponseSchema } from './research-contracts';
...
describe('@ivory-tower/contracts research contracts', () => {
    ...
    it('citation anchors require a sha256 content hash', () => {
        expect(() =>
            citationResolutionResponseSchema.parse({
                citationId: 'c1', projectId: 'n5-demo', resolvedAt: '2026-09-07T00:00:00.000Z',
                anchor: { sourcePath: 'research.py', sourceRevision: 'rev-1', anchorId: 'a1',
                    passage: 'x', startOffset: 0, endOffset: 1, contentHash: 'not-hex' },
            }),
        ).to.throw();
    });
    it('a resolved RunSpec carries protocol ref, environment, commands, and the run id', () => {
        const spec = resolvedRunSpecSchema.parse({
            runId: 'run-abc', projectId: 'n5-demo', revision: 'rev-1', sourceSetVersion: 'fixtures-rev-1',
            environment: { node: 'v22' }, commands: ['python research.py'],
    ...
    it('run-spec resolution may omit the semantic result', () => {
        const response = runSpecResolutionResponseSchema.parse({
            runId: 'run-abc', resolvedRunSpec: {
                runId: 'run-abc', projectId: 'n5-demo', revision: 'rev-1',
                sourceSetVersion: 'fixtures-rev-1', environment: {}, commands: [],
                resolvedAt: '2026-09-07T00:00:00.000Z',
            },
        });
    ...
    it('project edits require a base revision and validate edit ranges', () => {
        expect(() => projectEditRequestSchema.parse({
            projectId: 'n5-demo', baseRevision: '', sourcePath: 'research.py',
    ...
    it('conflict payloads carry authoritative state and an unchanged rejection echo', () => {
        ...
        const conflict = projectEditConflictSchema.parse({
            code: 'revision_conflict', baseRevision: 'rev-1', headRevision: 'rev-2',
            authoritative: { sourcePath: 'research.py', revision: 'rev-2', bytes: 'y',
