...
## R4 remainder after the 192-record benchmarks — executed and retained

The user requested completion of remaining gates. [New detailed remainder closeout](jev-r4-remainder-closeout.md) and [limited R5 disposition](jev-r5-bounded-disposition.md) supersede any earlier implication that *every* R4 dependency was simply an open plan:

- **48/48 isolated experimental tests PASS:** [workflow 35952000544](https://github.com/mberrys/ivory/actions/runs/35952000544) covers current review/receipt journal, per-request digest protection, 6 reviewer-provenance tests and existing 28 safety cases. No product Core change.
- **Actual original archived-N1 source kernel verified a separate NEW complete synthetic exact citation**, then actual local MiniLM generated a typed receipt and immutable archive snapshot remained unchanged, experimental journal reopened with CLI/Studio read parity. [Run 35952242656](https://github.com/mberrys/ivory/actions/runs/35952242656), artifact `10789286081`: 24 input tokens, 49.374 ms CPU inference, proposed `supported` at uncalibrated .990306, **zero human decisions and zero canonical research writes**. The DIFFERENT ORIGINAL archived advising N1 cite stayed `BLOCKED/context-unavailable` and was not model-dispatched. Archive reference kernel is *not* the undecided V5 Core/N7/N2 implementation.
- **Blinded human-review worklist generated with no real votes**: corrected [run 35952537458](https://github.com/mberrys/ivory/actions/runs/35952537458), artifact `10788913873` holds 120 untouched record IDs/text hashes and blank separate reviewer CSVs; initial run `35952000614` had an indirect blinded-label leak (an upstream labeled CSV URL in reviewer rows) and is superseded. The current packet excludes that URL and warns rights-authorized curator to supply historical-label-free original abstracts separately. **Actual human votes and adjudications: zero**.
- **Still genuinely outside automated completion:** independently performed blinded researcher ratings, selected production V5 Core+N7 authentic grants and N2 CAS+SQL store, interruption/restart across a real writer, two *deployed* CLI/Studio clients, target workstation (RTX 3090) memory/latency, licensed offline distribution and incurred billing. Experimental hash-chain JSONL and hardcoded test caller cannot certify those. No full 5-way scholarly-support calibration from 18 authored synthetics.

**R4 measured/isolated experimentation = complete. Full R4 academic and production qualification = NOT PASSED, not independently certifiable from current sources.** Do not adopt tested model-based hard exclusion/routing recipes or close R5 final architecture ADR. Keep the [Notion Jev epic](https://app.notion.com/p/3e49cb079ddb801db7ddda66ae715eaa) open until human and actual V5 owner deliverables exist.
