...
The machine contract currently records: one-Core authority as inherited; Claim Card authority as amended to a non-canonical projection; Research Capsule independent-reproduction qualification as deferred to Q3; V3 ORX as the current architecture qualification target as superseded by ADR-007 while retaining its one-Core invariants; and the semantic Assessment carrier as deferred to tracked issue V41-I03.2. None of these lineage entries closes a downstream qualification gate.
